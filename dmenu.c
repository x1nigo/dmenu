...
setup(void)
{
	...
	for (j = 0; j < SchemeLast; j++)
		scheme[j] = drw_scm_create(drw, colors[j], alphas[0], 2);
	...
	inputw = mw / 7; /* input width: ~70% of monitor width */
	match();
...
usage(void)
{
	die("usage: dmenu [-bFfivP] [-l lines] [-h height] [-p prompt] [-fn font] [-m monitor]\n"
